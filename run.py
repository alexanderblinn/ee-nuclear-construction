...
def conditions(row, date_limit):
    if row["Baubeginn"] <= date_limit:
        if (
            pd.isnull(row["erste Netzsynchronisation"])
            and pd.isnull(row["Kommerzieller Betrieb"])
            and pd.isnull(row["Abschaltung"])
            and pd.isnull(row["Bau/Projekt eingestellt"])
        ):
            return True
        else:
            if (not pd.isnull(row["erste Netzsynchronisation"])) and row[
                "erste Netzsynchronisation"
            ] >= date_limit:
                return True
            elif (not pd.isnull(row["Kommerzieller Betrieb"])) and row[
                "Kommerzieller Betrieb"
            ] >= date_limit:
                return True
            elif (not pd.isnull(row["Bau/Projekt eingestellt"])) and row[
...
def process_data(df: pd.DataFrame) -> dict[int, pd.DataFrame]:
    ...
    for year in years:
        ...
        data_year_grouped = data_year.groupby(
            by=["Land", data_year["Bau/Projekt eingestellt"].notnull()]
        ).size()
...
def plot_data(data: dict[int, pd.DataFrame]) -> None:
    ...
    fig.update_layout(
        ...
        yaxis=dict(
            title="Number of Nuclear Reactors under Construction",
            showgrid=True,
            gridwidth=1,
            gridcolor="rgba(128, 128, 128, 0.1)",
        ),
    ...
    fig.add_annotation(
        ...
        xanchor="left",
        yanchor="top",
        text="Construction Completed or Underway",
        showarrow=False,
    )
    ...
    fig.add_annotation(
        ...
        xanchor="left",
        yanchor="top",
        text="Construction Later Abandoned or Never Commissioned",
        showarrow=False,
    )
